...
impl Debug for ForwardBackwardIterator {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("ForwardBackwardIterator")
            .field("low_nonce_iter", &self.low_nonce_iter)
            .field("high_nonce_iter", &self.high_nonce_iter)
            .field("_domain", &self._domain)
            .finish()
    }
}
...
impl ForwardBackwardIterator {
    ...
    fn new(db: Arc<dyn HyperlaneDb>) -> Self {
        ...
        let mut low_nonce_iter =
            DirectionalNonceIterator::new(high_nonce, NonceDirection::Low, db.clone(), domain.clone());
    ...
    async fn try_get_next_message(
        ...
        loop {
            ...
            match (high_nonce_message_status, low_nonce_message_status) {
                ...
                // If both iterators give us unindexed messages, re-check the DB
                // for newly indexed messages. This handles the case where the index.from
                // config doesn't cover all historical nonces — the contract_sync may have
                // indexed newer messages that we haven't seen yet.
                (MessageStatus::Unindexed, MessageStatus::Unindexed) => {
                    if let Some(db_high_nonce) = self.db.retrieve_highest_seen_message_nonce().ok().flatten() {
                        let current_high = self.high_nonce_iter.nonce.unwrap_or_default();
                        if db_high_nonce > current_high {
                            debug!(
                                current_high,
                                db_high_nonce,
                                domain = self._domain,
                                "Re-syncing ForwardBackwardIterator: DB has newer nonces than current iterator position"
                            );
                            // Reset the high nonce iterator to start from the DB's highest nonce
                            self.high_nonce_iter = DirectionalNonceIterator::new(
                                db_high_nonce.into(),
                                NonceDirection::High,
                                self.db.clone(),
                                self._domain.clone(),
                            );
                            // Reset the low nonce iterator to scan backwards from the highest nonce
                            self.low_nonce_iter = DirectionalNonceIterator::new(
                                Some(db_high_nonce),
                                NonceDirection::Low,
                                self.db.clone(),
                                self._domain.clone(),
